...
The sentinel is checked under ``web/dist/`` (Vite manifest or index.html).
"""
...
class TestWebUIBuildNeeded:
    ...
    def test_stale_when_only_packaged_web_dist_not_web_dist(self, tmp_path):
        """Manifest solo en hermes_cli/web_dist no cuenta: el sentinel es web/dist."""
        web_dir, dist_dir = _make_web_dir(tmp_path)
        _touch(web_dir / "src" / "App.tsx", offset=-10)
        wrong = tmp_path / "hermes_cli" / "web_dist" / ".vite" / "manifest.json"
        _touch(wrong)
        assert _web_ui_build_needed(web_dir) is True
